...
from src.Runner import Causal_KoBert_Runner, KoBert_Runner
from src.model import Causal_Kobert, Kobert
...
def main(args) :
    ...
    base_model = Kobert.from_pretrained(model_config, config=config)
    base_model.to(device)
    base_run = KoBert_Runner(base_model, args.loss_weight)
    ...
    for i in range(1,args.epochs+1):
        ...
        base_run.train(fixed_batches, learning_rate =  args.lr)

    Cgs, CQ_probs, CYs, CTs = causal_run.inference(fixed_batches)
    Qs,Ys,Ts = base_run.inference(fixed_batches)

    results = pd.DataFrame({
        'prob': Cgs,    # prob taht trt = 1 
        'C0predict': np.exp(CQ_probs[:,0]), # if trt = 1 predict reaction
        'C1predict': np.exp(CQ_probs[:,1]), # if trt = 0 predict reaction
        'predict': np.exp(Qs),  # basic KoBert model predict reaction 
        'outcome1': np.exp(CYs),    # outcome1 == outcome2
        'outcome2': np.exp(Ys), 
        'gender1' : CTs,   # gender1 == gender2
        'gender2' : Ts
    })

    assert (results['outcome1'] == results['outcome2']).all() and \
        (results['gender1'] == results['gender2']).all(),\
         "Columns differ — stopping execution."

    ATT_hypothsis(results,0.05)
